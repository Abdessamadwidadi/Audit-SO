...
const STORE = { 
  ...
  READ_NOTIFS: 'mgtso_v1_read_notifs'
};
...
const App: React.FC = () => {
  ...
  const [readNotifIds, setReadNotifIds] = useState<string[]>(() => {
    const saved = localStorage.getItem(STORE.READ_NOTIFS);
  ...
  useEffect(() => {
    localStorage.setItem(STORE.READ_NOTIFS, JSON.stringify(readNotifIds));
  }, [readNotifIds]);
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key >= '0' && e.key <= '9') {
        ...
        handleLoginAttempt(pinInput);
      }
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [loginStep, pinInput, handleLoginAttempt]);

  const notifications = useMemo(() => {
    if (!currentUser) return [];
    const list = [];
    const today = new Date().toISOString().split('T')[0];
    const othersTasks = tasks.filter(t => String(t.assignedToId) === String(currentUser.id) && String(t.assignedById) !== String(currentUser.id) && t.status === 'todo');
    othersTasks.forEach(t => {
      const nid = `task-${t.id}`;
      if (!readNotifIds.includes(nid)) {
        const assigner = collaborators.find(c => String(c.id) === String(t.assignedById));
        list.push({ id: nid, type: 'task', title: 'MISSION REÇUE', msg: t.title, detail: assigner ? `Assignée par ${assigner.name}` : "Assignée par Manager", icon: <List size={18}/>, color: 'bg-blue-600' });
      }
    });
    const todayAtt = attendance.find(a => a.date === today && String(a.collaboratorId) === String(currentUser.id));
    const clockNid = `clock-missing-${today}`;
    if (!todayAtt && !readNotifIds.includes(clockNid)) {
      list.push({ id: clockNid, type: 'clock', title: 'POINTAGE MANQUANT', msg: 'Pensez à pointer votre arrivée.', detail: today, icon: <Clock size={18}/>, color: 'bg-rose-500' });
    }
    const todayEntries = entries.filter(e => e.date === today && String(e.collaboratorId) === String(currentUser.id));
    const logNid = `log-missing-${today}`;
    if (todayEntries.length === 0 && !readNotifIds.includes(logNid)) {
      list.push({ id: logNid, type: 'log', title: 'SAISIE MANQUANTE', msg: 'Vous n\'avez pas encore saisi vos heures aujourd\'hui.', detail: today, icon: <PlusCircle size={18}/>, color: 'bg-orange-500' });
    }
    return list;
  }, [tasks, attendance, currentUser, collaborators, entries, readNotifIds]);

  const markAsRead = (id: string) => setReadNotifIds(prev => [...new Set([...prev, id])]);
  const markAllAsRead = () => setReadNotifIds(prev => [...new Set([...prev, ...notifications.map(n => n.id)])]);

  const currentHistoryWeek = useMemo(() => {
  ...
  const filteredCollaborators = useMemo(() => {
    ...
    if (poleFilter !== 'all') list = list.filter(c => c.department?.toLowerCase() === poleFilter.toLowerCase());
    if (collabSearchQuery.trim()) {
      const s = collabSearchQuery.toLowerCase();
      list = list.filter(c => c.name.toLowerCase().includes(s));
    }
    return list;
  }, [collaborators, poleFilter, collabSearchQuery]);

  const filteredFolders = useMemo(() => {
    let list = folders;
    if (poleFilter !== 'all') list = list.filter(f => f.serviceType?.toLowerCase() === poleFilter.toLowerCase());
    if (folderSearchQuery.trim()) {
      const s = folderSearchQuery.toLowerCase();
      list = list.filter(f => f.name.toLowerCase().includes(s) || f.number.toLowerCase().includes(s));
    }
    return list;
  }, [folders, poleFilter, folderSearchQuery]);
        ...
        <div className="min-h-screen bg-[#020617] flex items-center justify-center p-6 text-white overflow-hidden">
          <div className="bg-white rounded-[4rem] w-full max-w-[440px] p-12 text-center shadow-2xl animate-in zoom-in">
            ...
            <div className="bg-[#eff6ff] rounded-[3.5rem] p-12 mb-16 flex items-center justify-center gap-6 relative">
              <div className="flex gap-4">{[...Array(6)].map((_, i) => (<div key={i} className={`w-5 h-5 rounded-full transition-all duration-200 ${pinInput.length > i ? 'bg-black' : 'bg-transparent'}`} />))}</div>
  ...
  return (
    <div className="min-h-screen flex bg-[#f8fafc]">
        ...
        <header className="mb-12 border-b border-slate-100 pb-10 flex justify-between items-end">
          ...
          <div className="flex items-center gap-6">
            <div className="relative">
              ...
              {showNotifPanel && (
                <div className="absolute right-0 top-16 w-96 bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 p-8 z-[200] animate-in slide-in-from-top-4">
                  <div className="flex justify-between items-center mb-6">
                    <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Alertes de gestion</p>
                    {notifications.length > 0 && (<button onClick={markAllAsRead} className="text-[9px] font-black text-indigo-600 uppercase tracking-widest hover:text-slate-900 transition-colors">Tout masquer</button>)}
                  </div>
                  <div className="space-y-4 max-h-[450px] overflow-y-auto hide-scrollbar">
                    {notifications.length === 0 ? (
                      <div className="text-center py-16"><CheckCircle2 size={48} className="mx-auto text-emerald-100 mb-4" /><p className="text-slate-300 font-black text-[10px] uppercase tracking-widest">Tout est à jour</p></div>
                    ) : notifications.map(n => (
                      <div key={n.id} className="flex gap-4 p-5 rounded-[2rem] bg-slate-50 border border-slate-100 hover:border-blue-300 transition-all group relative">
                        ...
                        <button onClick={() => markAsRead(n.id)} className="absolute top-4 right-4 p-2 text-slate-300 hover:text-indigo-600 transition-colors" title="Marquer comme lu"><Check size={16}/></button>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {isAdminOrManager && (
              <div className="flex bg-white p-1 rounded-2xl border border-slate-200 shadow-sm">
                {['all', ServiceType.AUDIT, ServiceType.EXPERTISE].map(p => (
                  <button key={p} onClick={() => setPoleFilter(p)} className={`px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${poleFilter === p ? (p?.toLowerCase() === 'audit' ? 'bg-blue-600 text-white shadow-md' : p?.toLowerCase() === 'expertise' ? 'bg-orange-500 text-white shadow-md' : 'bg-slate-900 text-white shadow-md') : 'text-slate-400 hover:bg-slate-50'}`}>{p === 'all' ? 'Global' : p}</button>
                ))}
              </div>
            )}
          </div>
              ...
              <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-4 bg-white px-6 py-2 rounded-2xl border border-slate-100 shadow-sm">
                    <button onClick={() => setHistoryWeekOffset(prev => prev - 1)} className="p-1 text-slate-400 hover:text-indigo-600 transition-colors"><ChevronLeft size={18}/></button>
                    <div className="text-center min-w-[220px]"><p className="text-[10px] font-black text-slate-900 uppercase tracking-widest">{formatDateFR(currentHistoryWeek.start)} — {formatDateFR(currentHistoryWeek.end)}</p></div>
                    <button onClick={() => setHistoryWeekOffset(prev => prev + 1)} className="p-1 text-slate-400 hover:text-indigo-600 transition-colors"><ChevronRight size={18}/></button>
                  </div>
                  ...
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-slate-50">{filteredHistory.map(e => (
                      <tr key={e.id} className="text-xs hover:bg-blue-50/20 text-slate-900 transition-all">
                        ...
                        <td className={`p-6 font-bold ${e.service?.toLowerCase() === 'audit' ? 'text-blue-600' : 'text-orange-500'}`}>{e.collaboratorName}</td>
                        ...
                        <td className={`p-6 text-center font-black text-lg ${e.service?.toLowerCase() === 'audit' ? 'text-blue-600' : 'text-orange-500'}`}>{e.duration}h</td>
                        <td className="p-6 text-right space-x-2">
                          ...
                          <button onClick={() => setDeleteConfirm({id: e.id, table: 'time_entries', label: `la saisie de ${e.collaboratorName} sur ${e.folderName}`})} className="p-2 text-slate-300 hover:text-rose-600 bg-slate-50 rounded-lg transition-colors"><Trash2 size={16}/></button>
                        </td>
                      </tr>
                    ))}</tbody>
                  </table>
                  {filteredHistory.length === 0 && <div className="p-20 text-center text-slate-300 uppercase text-[9px] font-black tracking-widest italic">Aucune saisie pour cette période</div>}
                </div>
              </div>
            </div>
          )}
              ...
              <div className="p-8 border-b bg-slate-50 flex flex-col md:flex-row justify-between items-center gap-6">
                <div className="flex flex-col gap-1">
                  ...
                  <div className="relative mt-2">
                    ...
                    <input type="text" placeholder="Rechercher collaborateur..." className="pl-12 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl font-bold text-[11px] text-slate-900 outline-none w-64" value={collabSearchQuery} onChange={e => setCollabSearchQuery(e.target.value)} />
                  </div>
                </div>
            ...
            <div className="bg-white rounded-[3rem] shadow-xl overflow-hidden border border-slate-200 animate-in fade-in">
              <div className="p-8 border-b bg-slate-50 flex flex-col md:flex-row justify-between items-center gap-6">
                <div className="flex flex-col gap-1">
                  ...
                  <div className="relative mt-2">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={16}/>
                    <input type="text" placeholder="Numéro ou nom dossier..." className="pl-12 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl font-bold text-[11px] text-slate-900 outline-none w-64" value={folderSearchQuery} onChange={e => setFolderSearchQuery(e.target.value)} />
                  </div>
                </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredFolders.map(f => (
                      <tr key={f.id} className="text-xs hover:bg-blue-50/20 transition-all">
                        <td className={`p-8 font-black ${f.serviceType?.toLowerCase() === 'audit' ? 'text-blue-600' : 'text-orange-500'}`}>{f.number}</td>
                        <td className="p-8 font-bold text-slate-900 text-sm">{f.name}</td>
                        <td className="p-8"><span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${f.serviceType?.toLowerCase() === 'audit' ? 'bg-blue-600 text-white' : 'bg-orange-500 text-white'}`}>{f.serviceType}</span></td>
                        <td className="p-8 text-right space-x-2">
      ...
      {deleteConfirm && <ConfirmModal title="Confirmer suppression ?" message={`Voulez-vous vraiment supprimer ${deleteConfirm.label} ?`} onConfirm={async () => { await supabase.from(deleteConfirm.table).delete().eq('id', deleteConfirm.id); setDeleteConfirm(null); fetchData(); showNotif('success', "Supprimé"); }} onCancel={() => setDeleteConfirm(null)} />}
    </div>
  );
};
